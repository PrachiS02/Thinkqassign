package com.merge;

import java.util.Arrays;

public class Mergearray
{
	public static int[] completemerge(int a[], int b[])
	{
		int mar[] = new int[a.length + b.length];
		int index = 0;

		for (int i = 0; i < a.length; i++) {
			mar[index++] = a[i];
		}

		for (int i = 0; i < b.length; i++) {
			mar[index++] = b[i];
		}
		return mar;

	}

	public static int[] alternatemerge(int a[], int b[])
	{
		int mar[] = new int[a.length + b.length];
		int index = 0;
		int maxlength = Math.max(a.length, b.length);

		for (int i = 0; i < maxlength; i++) 
		{
			if (i < a.length) 
			{
				{
					mar[index++] = a[i];
				}

			}
			if (i < b.length)
			{
				{
					mar[index++] = b[i];
				}

			}

		}
		return mar;
	}
    	
	public static int[] alternatemergeskip(int a[],int b[])
	{
		int maxlength=Math.max(a.length, b.length);
		int mar[]=new int[maxlength];
		int index=0;
		int i=0;
		while(index<maxlength)
		{
			if(i<a.length)
			{
				mar[index++]=a[i];
				i++;
			}
			if(i<b.length)
			{
				mar[index++]=b[i];
				i++;
			}
		}
		return mar;
		
	}
	
  public static void main(String[] args)
  {
	  
	  int a[]= {1,2,3,4,5,6};
	  
	  int b[]= {11,12,13,14};
	  
	  System.out.println(Arrays.toString(a));
	  System.out.println(Arrays.toString(b));
	  
	  System.out.println("complete merge:");
	  System.out.println(Arrays.toString(completemerge(a,b)));
	  
	  System.out.println("alternate merge:");
	  System.out.println(Arrays.toString(alternatemerge(a,b)));
	  
	  System.out.println("alternate skip merge:");
	  System.out.println(Arrays.toString(alternatemergeskip(a,b)));
	  
	
  }
}
